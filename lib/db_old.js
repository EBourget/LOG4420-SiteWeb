...
function getDomaine(question)
{
	return getDomaineById(question.domaine);
}
...
function getMaxQuestion(idDomaine)
{
	max = 0;
	for(var i =0; i<getNombreQuestions(); i++)
		{
			if(getQuestionById(i).domaine == idDomaine)
				max++;
		}
	return max;
}
...
{
	var idQuestion = Math.floor(Math.random()*getNombreQuestions());
...
function getQuestionsExamen(sujets)
{
	...
	for(j=0;j<sujets.length; j++)
	{
		...
		for(var i =0; i<getNombreQuestions(); i++)
		{
			if(getDomaineById(getQuestionById(i).domaine) == monDomaine)
...
exports.getDomaine = getDomaine;
exports.getQuestion = getQuestion;
exports.getChoix = getChoix;
exports.getReponse = getReponse;
exports.getMaxQuestion = getMaxQuestion;
